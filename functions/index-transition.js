...
const LAUNCHER_BOOT_BUNDLE_KEY='cp2020_boot_bundle_payload';
const LAUNCHER_TARGET_URL='DND.html?v=20260415e';
...
function clearLauncherPayload(){
  ...
  sessionStorage.removeItem(LAUNCHER_BOOT_BUNDLE_KEY);
}

function setLauncherPayload(payload){
  clearLauncherPayload();
  if(payload.bundleData){
    sessionStorage.setItem(LAUNCHER_BOOT_BUNDLE_KEY,JSON.stringify(payload.bundleData));
...
function startChippinInTransition(payload){
  ...
  if(title)title.textContent=payload.rawText||payload.bundleData?'UPLINKING DOSSIER':'BUILDING CHARACTER SHELL';
  if(text)text.textContent=payload.rawText||payload.bundleData?'Parsing upload and routing into dossier...':'Writing blank dossier shell and syncing identity...';
